...
<math>           Do math
<mplay>          Plays a song from my computer - Dont put the .mp3
<msongs>         Shows the list of songs that are avalible
<URLplay>        Plays a song from an url
...
\`\`\``)
  ...
  if (message.content.startsWith(prefix + "say")) {
    ...
    let t = args[2]
    let d = args[3]
    message.channel.send(`Embed generated:white_check_mark:
=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
**Title:**${t}
**Color:**${c}
**Description:**${d}
...
//
  ...
  if (message.content.startsWith(prefix + "test")) {
    ...
    if (botlog) botlog.send('Hi botlog! :D')
      message.channel.send("Selfbot")
  ...
  if (message.content.startsWith(prefix + "purge")) {
    ...
    let messagecount = parseInt(args[0]);
                    ...
                    if (!messagecount > 0) return message.delete();
                    ...
                    message.channel.fetchMessages({
                      ...
                      .then(messages => {
                          ...
                          if (msg_array.length < 1) return message.delete();
      ...
      if (message.content.startsWith(prefix + "flipcoin")) {
...

       {
        ...
        if (message.content.startsWith(prefix + "editmessage")) {
          ...
          message.channel.messages.get(`${id}`).edit(`${mess}`)
       }}
...
\`\`\``
...

          {
            if (message.content.startsWith(prefix + "mplay")) {
              message.delete()
              let args = message.content.split(' ').slice(1)
              if (!args[0]) return message.channel.send({embed: { title: ":x:Error", "color": 16711680, description: `**${prefix}play [Song name]**.mp3 or **${prefix}msongs** to see the song list`}}).then(m => {m.delete(15000);})
              if (!message.guild) return;
              if (message.member.voiceChannel) {
                message.member.voiceChannel.join().then(connection => {
          message.reply('I have successfully connected to the channel!').then(m => {m.delete(5000)});
          message.channel.send(`:white_check_mark:Now playing:\`\`\`${args.join(" ")}\`\`\``).then(m => {m.delete(5000);})
          if (botlog) botlog.send(`${message.author.username} has played the pc music: \`\`${args.join(' ')}\`\` in the channel \`\`${message.channel.name}\`\` in the server \`\`${message.guild.name}\`\``)
          const dispatcher = connection.playFile(`C:/Users/USER/Desktop/Discord Bot/Music/${args.join(' ')}.mp3`);
        });
              } else {message.reply('You need to join a voice channel first!').then(m => {m.delete(5000)});
          }}

          {
            if (message.content.startsWith(prefix + "msongs")) {
              message.delete();
              fs.readdir("C:/Users/USER/Desktop/Discord Bot/Music",function(err, files){
                if (err) {
                   return console.error(err);
                }
                let songs = (files.join('\n'));
                message.channel.send({embed: { title: "Song list :page_facing_up:", "color": 1891583, description: `**${songs}**`}})
              if (botlog) botlog.send(`${message.author.username} has requested the songs in the channel \`\`${message.channel.name}\`\` in the server \`\`${message.guild.name}\`\``)
            })}}

          {
            if (message.content.startsWith(prefix + "URLplay")) {
              message.delete()
              return message.channel.send("Command temporaly disabled.")
              let args = message.content.split(' ').slice(1)
              if (!args[0]) return message.channel.send({embed: { title: ":x:Error", "color": 16711680, description: `**${prefix}URLplay [Url].mp3**`}}).then(m => {m.delete(15000);})
              if (!message.guild) return;
              message.reply('Downloading music... Please wait...').then(m => {m.delete(5000)});
              if (message.member.voiceChannel) {
                message.member.voiceChannel.join().then(connection => {
          message.reply('I have successfully connected to the channel!').then(m => {m.delete(5000)});
          if (botlog) botlog.send(`${message.author.username} has played music in the url \`\`${args.join(' ')}\`\` in the channel \`\`${message.channel.name}\`\` in the server \`\`${message.guild.name}\`\``)
            const dispatcher = connection.playFile(`C:/Users/USER/Desktop/Discord Bot/Music/Song.mp3`);
        });
              } else {message.reply('You need to join a voice channel first!').then(m => {m.delete(5000)});
          }}}
...

              {
...

                {
                  if (message.content.startsWith(prefix + "messagebox")) {
                    message.delete();
                    let args = message.content.split(' ').slice(1)
                    if (!args[0]) return message.channel.send({embed: { title: ":x:Error", "color": 16711680, description: `Code is missing, Command:**${preifx}messagebox [code language] [Text]**`}}).then(m => {m.delete(10000);})
                    if (!args[1]) return message.channel.send({embed: { title: ":x:Error", "color": 16711680, description: `Text is missing, Command:**${preifx}messagebox [code language] [Text]**`}}).then(m => {m.delete(10000);})
                    let code = args[0]
